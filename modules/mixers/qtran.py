...
class QTranBase(nn.Module):
    ...
    def forward(self, batch, hidden_states, isTargetMixer, actions=None):
        bs = batch.batch_size
        ts = batch.max_seq_length - 1

        batch_data = batch.transition_data
        if(isTargetMixer):
            state = batch_data["state"][:,:-1]
            action_onehot = batch_data["actions_onehot"][:,:-1]
        else:
            state = batch_data["state"][:, 1:]
            action_onehot = batch_data["actions_onehot"][:, 1:]
        ...
        states = state.reshape(bs * ts, self.state_dim)

        if self.arch == "coma_critic":
            if actions is None:
                # Use the actions taken by the agents
                actions = action_onehot.reshape(bs * ts, self.n_agents * self.n_actions)
            else:
