...
class GNN(nn.Module):
    def __init__(self, num_input_features, hidden_layers, state_dim, hypernet_embed,
        ...
        for i in range(len(hidden_layers)):
            if i == 0:
                in_features = num_input_features # 1
            else:
            ...
            if combine_type == 'gin':
                self.layers.append(GINGraphConvolution(in_features, out_features, state_dim, hypernet_embed,
                                                       weights_operation=weights_operation)) # first output(992,3,32)
            else:
    ...
    def node_embedding(self, input_features, adj, states):
        ...
        return x
                    ...
                    # (32*31,3,1),  (32*31,3,3)
    def forward(self, input_features, adj, states, num_agents): # accept Q value of every agent
        batch_size = input_features.size(0) # (batch_size * num_seq) (992)
        output_features = self.node_embedding(input_features, adj,
                                              states)  # [:, :num_agents] # discard the outputs of entities  # (992,3,32)
        # readout, alive_agents_flags_1d = self.readout(output_features, adj, num_agents)
        readout = self.readout(output_features, adj, num_agents) #(992,32)

        wout = self.wout(states).view(batch_size, -1, 1) # (20,32,1)
        wout_perNode = self.wout_perNode(states).view(batch_size, -1, 1) # (20,32,1)
        bout_perNode = self.bout_perNode(states).view(batch_size, -1, 1).repeat(1, num_agents, 1) # (20,16,1)
        ...
        scalar_out = torch.matmul(readout.view(batch_size, 1, -1), wout) # （992,1,32） matmul (992,32,1) = （992,1,1）
        per_node_scalars = torch.matmul(output_features.view(batch_size, num_agents, -1), wout_perNode) + bout_perNode # (992,3,32) matmul (992,32,1) -> (992,3,1)
        # per_node_scalars = nn.Softmax(dim=1)(per_node_scalars + (-1e10 * (1 - 1. * alive_agents_flags_1d))) # (992, 3, 1)
        per_node_scalars = nn.Softmax(dim=1)(per_node_scalars) # (992, 3, 1)
        # per_node scalers = 每一个node 的 hidden feature * state transform 的 weight + bias -> nn.softmax = 每一个agent/node 的 contribution
    ...
    def readout(self, x, adj, num_agents):
        ...
        out = torch.sum(x, dim=1) / num_agents  # (992,32)
