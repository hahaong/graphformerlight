...
class TrafficSignal:
    ...
    def __init__(
        ...
        self.links = self.sumo.trafficlight.getControlledLinks(self.id) # have duplicate, a list contain [[(road1,roada,intersection)], [(road1,roadb,intersection)], [(road1,roadc,intersection)],.....]

        self.lane_dict = {} # include incoming lane and outgoing lane {'roada':['road1',....],....}
        for lane in self.links:
            incoming, outgoing, _ = lane[0] # unpack tuple
            if incoming not in self.lane_dict:
                self.lane_dict[incoming] = []
            self.lane_dict[incoming].append(outgoing)

        self.out_lanes = list(set(self.out_lanes)) # remove duplicate out lanes, but not keep in order
    ...
    def update(self):
        ...
        if self.is_yellow and self.time_since_last_phase_change == self.yellow_time:
            ...
            self.sumo.trafficlight.setRedYellowGreenState(self.id, self.all_phases[self.green_phase].state) # the yellow phase reached the max duration, now swap to the new green phase
            self.is_yellow = False
    ...
    def _queue_normalized_reward(self):
        return -self.get_total_queued_normalized()
    ...
    def _waiting_time_reward(self):
        ts_wait = sum(self.get_accumulated_waiting_time_per_lane())
        reward = -ts_wait
        return reward

    def _waiting_time_normalized_reward(self):
        ts_wait = sum(self.get_accumulated_waiting_time_per_lane()) / 100.0
        reward = -ts_wait
        return reward
    ...
    def get_average_speed_per_lane(self) -> float:
        """Returns the average speed normalized by the maximum allowed speed of the vehicles in the intersection.

        Obs: If there are no vehicles in the intersection, it returns 1.0.
        """
        veh_avg_speed_per_lane = []

        for lane in self.lanes:
            veh_list = self.sumo.lane.getLastStepVehicleIDs(lane)
            avg_speed = 0.0
            if len(veh_list) == 0:
                veh_avg_speed_per_lane.append(1.0)
            else:
                for v in veh_list:
                    avg_speed += self.sumo.vehicle.getSpeed(v) / self.sumo.vehicle.getAllowedSpeed(v)
                veh_avg_speed_per_lane.append(avg_speed / len(veh_list))

        return veh_avg_speed_per_lane
    ...
    def get_pressure_per_lane(self):
        pressure_per_incoming_lane = []

        for incoming_lane, outgoing_lanes in self.lane_dict.items():
            incoming_vehicles = self.sumo.lane.getLastStepVehicleNumber(incoming_lane)
            outgoing_vehicles = 0
            for outgoing_lane in outgoing_lanes:
                outgoing_vehicles += self.sumo.lane.getLastStepVehicleNumber(outgoing_lane)

            pressure = incoming_vehicles - outgoing_vehicles

            pressure_per_incoming_lane.append(pressure)

        """Returns the pressure (#veh leaving - #veh approaching) for every incoming lane of an intersection."""

        return pressure_per_incoming_lane
    ...
    def get_lanes_num_cars(self) -> List[float]:
        """Returns the density [0,1] of the vehicles in the incoming lanes of the intersection.
        ...
        return [car_numbers for car_numbers in lanes_car_numbers]
    ...
    def get_lanes_queue(self) -> List[float]:
        ...
        lanes_queue = [
            ...
            / (self.lanes_length[lane] / (self.MIN_GAP + self.sumo.lane.getLastStepLength(lane))) # min gap + the mean length of vehicles which were on this lane in the last step [m]
            for lane in self.lanes
        ]
        return [min(1, queue) for queue in lanes_queue]


    def get_lanes_queue_in_numbers(self) -> List[int]:
        """Returns the queue [0,N] of the vehicles in the incoming lanes of the intersection.
    ...
    def get_total_queued_normalized(self) -> int:
        """Returns the total number of vehicles halting in the intersection."""
        lanes_queue = [
            self.sumo.lane.getLastStepHaltingNumber(lane)
            / (self.lanes_length[lane] / (self.MIN_GAP + self.sumo.lane.getLastStepLength(lane))) # min gap + the mean length of vehicles which were on this lane in the last step [m]
        ...
        lanes_queue = [min(1, queue) for queue in lanes_queue]

        return sum(lanes_queue)/len(lanes_queue)
    ...
    reward_fns = {
        ...
        "waiting-time-normalized": _waiting_time_normalized_reward,
        "queue-normalized": _queue_normalized_reward
    }
