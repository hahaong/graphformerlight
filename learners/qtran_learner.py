...
class QLearner:
    ...
    def train(self, batch, t_env: int, episode_num: int):
        ...
        if self.args.mixer == "qtran_base":
            ...
            joint_qs, vs = self.mixer(batch, mac_hidden_states[:,:-1], isTargetMixer=False) # batch need [:,:-1] inside
            ...
            target_joint_qs, target_vs = self.target_mixer(batch, hidden_states=target_mac_hidden_states[:,1:], isTargetMixer=True, actions=max_actions_onehot[:,1:])
            ...
            td_loss = (masked_td_error ** 2).sum() / num_data.sum()
            # -- TD Loss --
            ...
            if not self.args.double_q: # Already computed if we're doing double Q-Learning
                max_actions_current_ = th.zeros(size=(batch.batch_size, batch.max_seq_length, self.args.n_agents, self.args.n_actions), device=batch.device )
                max_actions_current_onehot = max_actions_current_.scatter(3, max_actions_current[:, :], 1)
            max_joint_qs, _ = self.mixer(batch, mac_hidden_states[:,:-1], isTargetMixer = False, actions=max_actions_current_onehot[:,:-1]) # Don't use the target network and target agent max actions as per author's email
            ...
            masked_opt_error = opt_error
            # masked_opt_error = opt_error * mask.reshape(-1, 1)
            num_data = th.tensor([1], device=masked_opt_error.device, dtype=th.uint8)
            num_data = num_data.expand_as(masked_opt_error)
            opt_loss = (masked_opt_error ** 2).sum() / num_data.sum()
            # -- Opt Loss --
            ...
            masked_nopt_error = nopt_error
            num_data = th.tensor([1], device=masked_nopt_error.device, dtype=th.uint8)
            num_data = num_data.expand_as(masked_nopt_error)
            nopt_loss = (masked_nopt_error ** 2).sum() / num_data.sum()
            # -- Nopt loss --
